...
public class SkinnedCollisionHelper : MonoBehaviour
{
    ...
    private MeshCollider meshCollider; // quick pointer to the mesh collider that we're updating
                                  // Function:    Start
    ...
    void Start()
    {
        SkinnedMeshRenderer renderer = GetComponent(typeof(SkinnedMeshRenderer)) as SkinnedMeshRenderer;
        meshCollider = GetComponent(typeof(MeshCollider)) as MeshCollider;
        if (meshCollider != null && renderer != null)
        {
            Mesh baseMesh = renderer.sharedMesh;
            mesh = new Mesh();
            ...
            nodeWeights = new CWeightList[renderer.bones.Length];
            for (i = 0; i < renderer.bones.Length; i++)
            {
                nodeWeights[i] = new CWeightList();
                nodeWeights[i].transform = renderer.bones[i];
            }
